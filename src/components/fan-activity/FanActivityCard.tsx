/**
 * Fan Activity Card Component
 * Displays live feed of recent fan interactions with collapsible UI
 */

import { useState } from 'react';
import { motion, AnimatePresence } from 'framer-motion';
import { Card } from '@/components/ui/Card';
import { FanActivityItem } from './FanActivityItem';
import { useFanActivity } from '@/hooks/useFanActivity';

export function FanActivityCard() {
  const [isExpanded, setIsExpanded] = useState(true);
  const activities = useFanActivity();
        ...
        {isExpanded && (
          ...
          >
            {activities.slice(0, 5).map((activity, index) => (
              <FanActivityItem key={activity.id} activity={activity} index={index} />
            ))}
          </motion.div>
        )}
      </AnimatePresence>
